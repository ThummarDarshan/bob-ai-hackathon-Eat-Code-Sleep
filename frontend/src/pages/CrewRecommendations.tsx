...
export default function CrewRecommendations() {
  ...
  useEffect(() => {
    Promise.all([
      getRecommendations(),
      // Fetch all work orders by loading recommendations first then assets
    ])
      .then(([recs]) => {
        setRecommendations(recs.work_orders ?? []);
        // Also fetch work orders from all assets mentioned
        ...
        {!loading && !error && (
          <>
            ...
            <div className="glass-card mb-6">
              <div className="section-title mb-4">Auto-Generated Work Order Recommendations</div>
                ...
                <tbody>
                  {recommendations.length === 0 ? (
                    ...
                    recommendations.map((r: any) => (
                      <tr key={r.asset_id}>
                        <td>
                          <span style={{ fontFamily: 'monospace', fontSize: '12px', color: 'var(--blue-glow)' }}>{r.asset_id}</span>
                        ...
                        <td style={{ fontSize: '12px' }}>{r.action}</td>
                      </tr>
                    ))
                  )}
                </tbody>
            ...
            {/* Crew staging plan */}
            <div className="glass-card mb-6">
              <div className="section-title mb-4">48-Hour Crew Staging Plan</div>
              <div className="grid-3 gap-4">
                {[
                  ...
                  <div key={hub.name} style={{ padding: '16px', background: 'rgba(15,23,42,0.6)', borderRadius: '10px', border: '1px solid var(--glass-border)' }}>
                    <div style={{ fontWeight: 600, marginBottom: '6px' }}>{hub.name}</div>
                    <div className="text-muted text-sm">Zone: {hub.zone}</div>
                    <div style={{ marginTop: '8px', fontSize: '13px' }}>
                      <span style={{ color: hub.critical > 0 ? 'var(--risk-critical)' : 'var(--green)' }}>
            ...
            {workOrders.length > 0 && (
              <div className="glass-card">
                <div className="section-title mb-4">Active Work Orders</div>
                <table className="data-table">
                  <thead>
