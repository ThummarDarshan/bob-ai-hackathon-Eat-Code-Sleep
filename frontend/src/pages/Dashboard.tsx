        ...
        <div className="grid-2 gap-6">
          ...
          <div className="glass-card">
            <div className="section-header">
              ...
              <button className="btn btn-ghost" style={{ fontSize: '11px' }} onClick={() => navigate('/risk')}>View All →</button>
            </div>
                ...
                <tbody>
                  {data!.top_risk_assets.map(a => (
                    <tr key={a.asset_id} style={{ cursor: 'pointer' }} onClick={() => navigate(`/assets/${a.asset_id}`)}>
                      ...
                      <td style={{ fontWeight: 600 }}>{a.final_risk_score !== undefined ? (a.final_risk_score * 100).toFixed(0) + '%' : '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="grid-2 gap-6 mt-4">
            ...
            {data!.recent_incidents.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {data!.recent_incidents.map((inc: any) => (
                  <div key={inc.id} style={{ display: 'flex', gap: '10px', padding: '10px', background: 'rgba(15,23,42,0.5)', borderRadius: '8px' }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: '12px', fontWeight: 600 }}>{inc.asset_id} — {inc.failure_type}</div>
                      <div className="text-muted text-sm" style={{ marginTop: '2px' }}>{inc.description?.slice(0, 80)}...</div>
                    </div>
          ...
          <div className="glass-card">
            <div className="section-header">
              ...
              <button className="btn btn-ghost" style={{ fontSize: '11px' }} onClick={() => navigate('/weather')}>Details →</button>
            </div>
            {data!.weather_alerts.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {data!.weather_alerts.map((alert: any) => (
                  <div key={alert.asset_id} style={{ padding: '10px', background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.2)', borderRadius: '8px', fontSize: '12px' }}>
                    <div style={{ fontWeight: 600, color: 'var(--amber)' }}>{alert.asset_name}</div>
